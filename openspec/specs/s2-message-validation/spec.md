# s2-message-validation Specification
...
### Requirement: Explicit null on an incoming field is normalized to absent before validation
No S2 schema accepts `null` as a valid value for any field. Since some CEM implementations serialize an absent optional field as explicit `null` rather than omitting the key, `s2-rm`'s session layer SHALL strip `null`-valued properties from an incoming CEM message before validating it, recursively, so such a field is treated as absent rather than failing validation with a type error. A `null` on a field that is actually required SHALL still fail validation (via the `required` check instead of `type`). The session SHALL warn at most once per session per distinct (message type, field) when a null is stripped, rather than on every occurrence.

#### Scenario: A CEM sends an optional field as explicit null
- **WHEN** an incoming message has an optional field (e.g. `diagnostic_label` on `ReceptionStatus`) set to `null` instead of omitted
- **THEN** that field is stripped before validation, the message validates as if the field were absent, and a warning is logged at most once per session for that (message type, field) pair

#### Scenario: A CEM sends a required field as explicit null
- **WHEN** an incoming message has a required field set to `null`
- **THEN** the field is stripped before validation, and the message still fails validation because the field is now missing, not because of its type
