...
- **WHEN** a user runs `npm install node-red-contrib-s2` and opens the edit dialog for any of `s2-rm`, `s2-websocket`, `s2-rm-config`, or `s2-cem-config` in the Node-RED editor
...
- **WHEN** `npm pack` is run against this package
- **THEN** the tarball contains `dist/nodes/<node-name>/index.html` for each of `s2-rm`, `s2-websocket`, `s2-rm-config`, and `s2-cem-config`
