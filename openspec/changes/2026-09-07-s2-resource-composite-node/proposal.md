...
- Add `s2-resource`: one node with a tabbed edit dialog (Connection / Resource Manager / Control Type) that internally instantiates the same session state machine `s2-rm` uses, plus a built-in transport and control type.
  - **Transport**: `WebSocket` (built-in - reuses `s2-cem-config`'s connection fields), `D-Bus` (built-in - reuses `s2-dbus-config`'s connection/device fields, via the same `S2DbusTransport` class `2026-09-06-s2-dbus-transport`'s `s2-dbus` node uses), or `External` (no built-in transport; exposes the same "to/from transport" input/output pair `s2-rm` has today, for wiring to anything else that speaks the transport-agnostic message protocol).
...
- `package.json`: `s2-resource` added to the `node-red.nodes` map. No dependencies new to this change specifically - `Transport: WebSocket` reuses the existing `ws` dependency, and `Transport: D-Bus` reuses `dbus-victron-virtual`/`dbus-native-victron` and `src/lib/transport/dbus.ts` added by `2026-09-06-s2-dbus-transport` (landing together with this change).
- No changes to `s2-rm`, `s2-websocket`, `s2-dbus`, `s2-cem-config`, `s2-dbus-config`, or any of their specs' requirements - `s2-resource` is additive and consumes them as-is.
